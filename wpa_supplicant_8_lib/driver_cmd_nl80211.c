...
#ifdef USE_64_BIT_IPC
    priv_cmd.buf = (uint64_t)(uintptr_t)buf;
#else
    priv_cmd.buf = buf;
#endif
    priv_cmd.used_len = (int)sizeof(buf);
    priv_cmd.total_len = (int)sizeof(buf);
...
static int hostapd_read_mac_addr_list(const char *file_path, u8 mac_addr_list[][ETH_ALEN])
{
  ...
  f = fopen(file_path, "r");
  if (f == NULL) return 0;
  mac_addr_list_len = (int)fread(mac_addr_list, ETH_ALEN, HOSTAP_MAX_BLOCK_NUM, f);
  fclose(f);
...
static int hostapd_write_mac_addr_list(const char *file_path, u8 mac_addr_list[][ETH_ALEN], int mac_addr_list_len)
{
  ...
  f = fopen(file_path, "w");
  if (f == NULL) return -1;
  if((int)fwrite(mac_addr_list, ETH_ALEN, mac_addr_list_len, f) != mac_addr_list_len) {
...
int hostapd_init_block_list(void *priv)
{
  ...
  for(i = 0; i < mac_addr_list_len; i++) {
      snprintf(cmd, sizeof(cmd), "BLOCK %02x:%02x:%02x:%02x:%02x:%02x", 
               mac_addr_list[i][0], mac_addr_list[i][1], mac_addr_list[i][2], 
               mac_addr_list[i][3], mac_addr_list[i][4], mac_addr_list[i][5]);
...
int wpa_driver_nl80211_driver_cmd(void *priv, char *cmd, char *buf,
  ...
  if (os_strcasecmp(cmd, "STOP") == 0) {
    ...
    int hdrlen = 2 * (int)sizeof(unsigned short);
    ...
    *(unsigned short *)(buf + hdrlen / 2) = (unsigned short)sizeof(max_sta_num);
    *(buf + hdrlen) = max_sta_num;
    buf_len = (size_t)(hdrlen + sizeof(max_sta_num));
...
#endif
//<-- Add for SoftAp Advance Feature END
    priv_cmd.used_len = (int)buf_len;
    priv_cmd.total_len = (int)buf_len;
    ifr.ifr_data = &priv_cmd;
    ...
    miracast->msg_len = (uint16_t)sizeof(int);
    os_memcpy(miracast->msg_data, &value, sizeof(value));

//NOTE: Bug#474462 Add for SoftAp Advance Feature BEG-->
#ifdef USE_64_BIT_IPC
    priv_cmd.buf = (uint64_t)(uintptr_t)miracast;
#else
    priv_cmd.buf = (char *)miracast;
#endif
//<-- Add for SoftAp Advance Feature END
    priv_cmd.used_len = (int)(sizeof(*miracast) + miracast->msg_len);
...
//NOTE: Bug#474462 Add for SoftAp Advance Feature BEG-->
      ...
      cmd_buf[9] = ' ';
      os_snprintf(cmd_buf+10, 18, MACSTR, MAC2STR((const u8*)&cmd[15]));
      ...
      while (i < hostapd->conf->num_accept_mac) {
          if (os_memcmp(acl[i].addr, addr, ETH_ALEN) == 0) {
              os_remove_in_array(acl, hostapd->conf->num_accept_mac, (int)sizeof(*acl), i);
              hostapd->conf->num_accept_mac--;
              break;
          }
          i++;
      ...
      ret = os_snprintf(buf, buf_len, "OK");
        ...
        if(sscanf(cmd+6, MACSTR, &mac_addr[0], &mac_addr[1], &mac_addr[2], 
        ...
        u8 mac_addr[ETH_ALEN];
        if(sscanf(cmd+8, MACSTR, &mac_addr[0], &mac_addr[1], &mac_addr[2], 
                  &mac_addr[3], &mac_addr[4], &mac_addr[5]) != 6)
            return -1;
        if(android_priv_cmd(priv, cmd) < 0)
            return -1;
        ...
        mac_addr_list_len = hostapd_read_mac_addr_list(HOSTAP_BLOCK_LIST_FILE, mac_addr_list);
        for(i = 0; i < mac_addr_list_len; i++) {
            ret = os_snprintf(buf + i * 18, buf_len - (size_t)(i * 18), 
                              MACSTR " ", MAC2STR(mac_addr_list[i]));
        }
...
int wpa_driver_nl80211_driver_cmd_wnm(void *priv, enum wnm_oper oper,
  ...
  priv_cmd.buf = buff;
  priv_cmd.used_len = (int)sizeof(buff);
  priv_cmd.total_len = (int)sizeof(buff);
  ifr.ifr_data = &priv_cmd;
...
int driver_init_max_sta_num(struct i802_bss *bss)
{
    char buf[15];
    struct hostapd_data *hostapd = bss->ctx;
    *buf = (char)hostapd->conf->max_num_sta;
