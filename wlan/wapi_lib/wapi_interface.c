...
int WIFI_lib_init(void)
{
    ...
    if (!s_init)
    {
        ...
        s_asue = (struct wapi_asue_st*)iwn_get_buffer(sizeof(struct wapi_asue_st));
        if (s_asue == NULL)
            return ret;
...
int WIFI_lib_exit(void)
{
    ...
    s_asue = (struct wapi_asue_st*)iwn_free_buffer(s_asue, sizeof(struct wapi_asue_st));
